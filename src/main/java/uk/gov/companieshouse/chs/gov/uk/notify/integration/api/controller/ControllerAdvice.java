...
public class ControllerAdvice extends ResponseEntityExceptionHandler {
    ...
    private static final String ACSP_MANAGE_USERS_API = "acsp_manage_users_api";
    private static final String QUERY_PARAMETERS = "query-parameters";

    private String getJsonStringFromErrors(final String requestId, final Errors errors) {
        final var objectMapper = new ObjectMapper();

        try {
            return objectMapper.writeValueAsString(errors);
        } catch (IOException exception) {
            LOG.errorContext(requestId, String.format("Fail to parse Errors object to JSON %s", exception.getMessage()), exception, null);
            return "";
        }
    }
    ...
    public Errors onConstraintViolationException(final ConstraintViolationException exception, final HttpServletRequest request) {
        ...
        for (final ConstraintViolation<?> constraintViolation : exception.getConstraintViolations()) {
            errorsToBeLogged.addError(Err.invalidBodyBuilderWithLocation(ACSP_MANAGE_USERS_API)
